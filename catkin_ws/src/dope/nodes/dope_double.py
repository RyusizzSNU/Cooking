...
class Draw(object):
    ...
    def draw_axis(self, center, rvec, tvec, mtx, dist=None):
        """
        Draws axis with a thick solid line.
        """
        axis = np.float32([[100,0,0], [0,100,0], [0,0,-100]]).reshape(-1,3)
        if dist is None:
            dist = np.zeros((1,5))

        rvec = np.float32(R.from_quat(rvec).as_rotvec()).reshape(-1,3)
        tvec = np.float32(tvec).reshape(-1,3)
        points, jac = cv2.projectPoints(axis, rvec, tvec, mtx, dist)
        center = tuple(center.ravel())
        self.draw_line(center, tuple(points[0].ravel()), (255,0,0))
        self.draw_line(center, tuple(points[1].ravel()), (0,255,0))
        self.draw_line(center, tuple(points[2].ravel()), (0,0,255))

class DopeDouble(object):
    def __init__(self):
        self.models = {}
        self.dope_L = Dope(self, 'L')
        self.dope_R = Dope(self, 'R')
...
class Dope(object):
    ...
    def __init__(self, parent, direction):
        self.parent, self.direction = parent, direction
        self.pubs = {}
        # self.models = {}
        self.pnp_solvers = {}
        self.pub_dimension = {}
        ...
        self.input_is_rectified = self.get_param_w_direction('~input_is_rectified', True)
        self.downscale_height = self.get_param_w_direction('~downscale_height', 500)
        ...
        self.config_detect.thresh_angle = self.get_param_w_direction('~thresh_angle', 0.5)
        self.config_detect.thresh_map = self.get_param_w_direction('~thresh_map', 0.01)
        self.config_detect.sigma = self.get_param_w_direction('~sigma', 3)
        self.config_detect.thresh_points = self.get_param_w_direction("~thresh_points", 0.1)

        # For each object to detect, load network model, create PNP solver, and start ROS publishers
        for model, weights_url in self.get_param_w_direction('~weights').items():
            # if DopeNode.models.get(model) is None:
            #     DopeNode.models[model] = \
            #         ModelData(
            #             model,
            #             resource_retriever.get_filename(weights_url, use_protocol=False)
            #         )
            #     DopeNode.models[model].load_net_model()

            try:
                M = np.array(self.get_param_w_direction('~model_transforms')[model], dtype='float64')
            ...
            try:
                self.meshes[model] = self.get_param_w_direction('~meshes')[model]
            except KeyError:
                pass

            try:
                self.mesh_scales[model] = self.get_param_w_direction('~mesh_scales')[model]
            except KeyError:
                self.mesh_scales[model] = 1.0
            try:
                self.draw_colors[model] = tuple(self.get_param_w_direction("~draw_colors")[model])
            except:
                self.draw_colors[model] = (np.random.randint(0,255),np.random.randint(0,255),np.random.randint(0,255))

            self.dimensions[model] = tuple(self.get_param_w_direction("~dimensions")[model])
            self.class_ids[model] = self.get_param_w_direction("~class_ids")[model]

            self.pnp_solvers[model] = \
                CuboidPNPSolver(
                    model,
                    cuboid3d=Cuboid3d(self.get_param_w_direction('~dimensions')[model])
                )
            self.pubs[model] = \
                rospy.Publisher(
                    '{}/pose_{}'.format(self.get_param_w_direction('~topic_publishing'), model),
                    PoseStamped,
                    queue_size=10
                )
            self.pub_dimension[model] = \
                rospy.Publisher(
                    '{}/dimension_{}'.format(self.get_param_w_direction('~topic_publishing'), model),
                    String,
                    queue_size=10
                )
        ...
        self.pub_rgb_dope_points = \
            rospy.Publisher(
                self.get_param_w_direction('~topic_publishing') + "/rgb_points",
                ImageSensor_msg,
                queue_size=10
            )
        self.pub_camera_info = \
            rospy.Publisher(
                self.get_param_w_direction('~topic_publishing') + "/camera_info",
                CameraInfo,
                queue_size=10
            )
        self.pub_detections = \
        ...
        image_sub = message_filters.Subscriber(
            self.get_param_w_direction('~topic_camera'),
            ImageSensor_msg
        )
        info_sub = message_filters.Subscriber(
            self.get_param_w_direction('~topic_camera_info'),
            CameraInfo
        )
        ts = message_filters.TimeSynchronizer([image_sub, info_sub], 1)
        ts.registerCallback(self.image_callback)

        print("Running DOPE...  (Listening to camera topic: '{}')".format(self.get_param_w_direction('~topic_camera')))
        print("Ctrl-C to stop")

    def get_param_w_direction(self, name, default=None):
    ...
    def image_callback(self, image_msg, camera_info):
        ...
        activities = self.get_param_w_direction('~activities').items()
        for i, m in enumerate(DopeNode.models):
            if not activities[i][1]:
                continue
            # Detect object
            results = ObjectDetector.detect_object_in_image(
                DopeNode.models[m].net,
                self.pnp_solvers[m],
                img,
                self.config_detect
            )
            ...
            for i_r, result in enumerate(results):
                ...
                if None not in result['projected_points']:
                    ...
                    for pair in result['projected_points']:
                        ...
                        points2d_pub.extend(pair)
                    draw.draw_cube(points2d, self.draw_colors[m])
                    draw.draw_axis(result['projected_points'][-1,:], ori, loc, camera_matrix, dist=dist_coeffs)
                    draw.draw_dot(result['projected_points'][-1,:], (255,255,255), 3)
                    self.pub_projected.publish(points2d_pub)
...
def main():
    ...
    # Initialize ROS node
    rospy.init_node('dope_L')
    DopeNode()
    rospy.init_node('dope_R')
    DopeNode()
